<?php
class Betube_MostViewed_MAG extends WP_Widget {

    public function __construct() {
        $widget_ops = array('classname' => 'Betube_MostViewed_MAG', 'description' => esc_html__( "Show Most Viewed Magzine Posts.", 'betube-helper') );
        parent::__construct('betube-mostviewed-mag', esc_html__('BeTube Most Viewed Magzine Posts', 'betube-helper'), $widget_ops);
        $this->alt_option_name = 'Betube_MostViewed_MAG';       
    }

    public function widget($args, $instance) {
        $cache = array();
        if ( ! $this->is_preview() ) {
            $cache = wp_cache_get( 'betube_widget_mag_mostviewed_posts', 'widget' );
        }
        ...
        $title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Most Viewed Posts', 'betube-helper' );
...
<?php
        ...
        if ( ! $this->is_preview() ) {
            ...
            wp_cache_set( 'betube_widget_mag_mostviewed_posts', $cache, 'widget' );
        } else {
            ob_end_flush();
        }
    }
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        if ( isset($alloptions['Betube_MostViewed_MAG']) )
            delete_option('Betube_MostViewed_MAG');
...
add_action('widgets_init', create_function('', 'return register_widget("Betube_MostViewed_MAG");'));